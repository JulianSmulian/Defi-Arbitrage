...
let eth_pairs = [
 ...
 //{ symbol: 'THIS', address: '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'}
];
...
async function getPair(symbol,tokenaddress, expectedtoken0, expectedtoken1,hash){
  const factoryPair = await uniswapFactoryContract.methods.getPair('0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2',tokenaddress).call()
  const erc20PairContract = new web3.eth.Contract(PAIR_ABI, factoryPair)
  let exchange_info = await erc20PairContract.methods.getReserves().call()
  let token0 = await erc20PairContract.methods.token0().call()
  if(token0.toLowerCase()=='0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'){
    let oldtoken   = web3.utils.fromWei(exchange_info[1])/web3.utils.fromWei(exchange_info[0])
    let new_token0 = web3.utils.fromWei(exchange_info[0])-web3.utils.fromWei(expectedtoken1)
    let new_token1 = web3.utils.fromWei(exchange_info[1])-web3.utils.fromWei(expectedtoken0)
    let ratio = new_token1/new_token0
    //console.log('RATIO:',ratio)
    if(ratio>0){
    let sushiswapAmountsOut = await sushiRouterContract.methods.getAmountsOut(web3.utils.toWei(ratio.toString()),([tokenaddress,'0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'])).call()
    if(toPlainString(web3.utils.fromWei(sushiswapAmountsOut[1]))>1){
      console.log('ARBI',symbol,ratio.toString(),web3.utils.fromWei(sushiswapAmountsOut[1]),hash,tokenaddress)
      if((web3.utils.fromWei(sushiswapAmountsOut[1])-1)>.004){
        alertTerminal()
        console.log('WORTH ARBITRAGE',(web3.utils.fromWei(sushiswapAmountsOut[1])-1)>.004)
      }
      }
      else{
        console.log(symbol, web3.utils.fromWei(sushiswapAmountsOut[1]),'is less than 1')
      }
    }
    //console.log('Previous token return for',symbol, oldtoken ,'New token ratio', ratio, 'Change is',(ratio-oldtoken), hash)
  }
    else{
  let oldtoken   = web3.utils.fromWei(exchange_info[0])/web3.utils.fromWei(exchange_info[1])
  let new_token0 = web3.utils.fromWei(exchange_info[0])-web3.utils.fromWei(expectedtoken0)
  let new_token1 = web3.utils.fromWei(exchange_info[1])-web3.utils.fromWei(expectedtoken1)
  let ratio = new_token0/new_token1
  //console.log('RATIO:',ratio)
  if(ratio>0){
  let sushiswapAmountsOut = await sushiRouterContract.methods.getAmountsOut(web3.utils.toWei(ratio.toString()),([tokenaddress,'0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'])).call()
  if(toPlainString(web3.utils.fromWei(sushiswapAmountsOut[1]))>1){
...
async function getUniswapPendingTransactions(address) {
    const pendingBlock = await web3.eth.getBlock("pending");
    const transactions = await Promise.all(pendingBlock.transactions.map(txHash => web3.eth.getTransaction(txHash)));
    //console.log('THERE ARE',transactions.length,'CURRENTLY')
    const pendingTransactions = transactions.filter(tx => tx && tx.to == address) //&& tx.to == CONTRACT_ADDRESS); How to watch any wallet
    //0 value means buying coin with ETH
    for (p in pendingTransactions){
    //console.log('Pending Transcation input', pendingTransactions[p].input)
    //console.log('Pending Transcation value', pendingTransactions[p].value)
    if(pendingTransactions[p].value==0||pendingTransactions[p].value >= 1){
      const trans_hash = pendingTransactions[p].hash
      //console.log('Pending Transcation value', pendingTransactions[p].value,'HASH:', trans_hash)
      let trans_value = pendingTransactions[p].value

      getTransaction(trans_hash,trans_value)


      }  }
      //console.log('Pending Transaction input', pendingTransactions[p].getInput())
    return pendingTransactions;
    }

async function getTransaction(hash,trans_value){

    await web3.eth.getTransaction(hash, function(err, tx){
    ...
    for(i in params){
      ...
      if(params[i].name=='path'){
        //console.log('Path',i,params[i].value)
        for(z in params[i].value){
        //console.log('PATHS',z, i,params[i].value[z],weth[0].address)
        //const p = params[i].value[z]
        for(s in eth_pairs){
          for(pv in params[i].value){
            if(params[i].value[pv].toLowerCase()==eth_pairs[s].address.toLowerCase()){
              if(params[i].value[z].toLowerCase()==weth[0].address){
                //console.log('POSITION',pv, eth_pairs[s].symbol,params[i].value[pv],trans_hash)
                //console.log('ETH POSITION',z, trans_hash)
                //if(pv=='0'&&z=='1'){console.log('coin is being sold/More coins being added to pair')}
                if(pv=='1'&&z=='0'){
                  //console.log('Swaping', web3.utils.fromWei(trans_value),' ETH for',web3.utils.fromWei(params[0].value),eth_pairs[s].symbol,trans_hash )}
                  getPair(eth_pairs[s].symbol,eth_pairs[s].address,params[0].value,trans_value,hash)
              }else{
                //if(pv=='2'&&z=='0'){console.log('coin is being bought by a different coin no eth involved/ removed from pair, possible arbitrage')}
              }
            //console.log('HASH:',trans_hash, 'SYMBOL:',eth_pairs[s].symbol, 'ETH VALUE:', trans_value, 'VALUE:',params[0].value);
          }
          }
        }
...
}catch(error){getTransaction(hash,trans_value)}


    });
}

async function getExpectedTokenPrice(){
  ...
  //numberOfEth   = GET NUMBER OF ETH FROM ETH/TOKEN PAIR getReserves then reserve 1
  //numberOfTokens = GET NUMBER OF TOKENS FROM ETH/TOKEN PAIR getReserves then reserve 0
  PriceOfToken = (ethPrice_inDai*numberOfEth) / numberOfTokens
  //                  2500*500 / 1000000 =1.25
  //                  >numofEth = > PriceOfToken  1%>numberOfEth = (2500*505) / 1.275 = 990000
  //                  >numberOfTokens = < PriceOfToken
  ratio = ethPrice_inDai/PriceOfToken
}

async function monitorTranscations() {
  ...
  try {
    ...
    getUniswapPendingTransactions(UNISWAP_ROUTER_ADDRESS)
  } catch (error) {
  console.error(error)
  return
  }
...
priceMonitor = setInterval(async () => { await monitorTranscations() }, 60000) //75 seconds 6 hours of infura
...
async function swapEth_SUSHI(trading_address,expectedAmount) {
  ...
  clearInterval(pairs_monitorTranscations)
